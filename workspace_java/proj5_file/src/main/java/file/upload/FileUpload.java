...
public class FileUpload extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			File currentDirPath = new File("C:\\workspace\\temp\\upload");
			...
			// request를 분석해서
			// 요청 파라메터르르 Filetem 뽑고 그걸 list에 담아서 돌려줌
			...
			for (int i = 0; i < items.size(); i++) {
				...
				if (fileItem.isFormField()) {
					...
					if (fileItem.getSize() > 0) {
						...
						File uploadFile = new File(currentDirPath + "\\" + fileName);  // File uploadFile : 경로를 만들어서 경로에 파일 이름을 넣는다 
						// "\\" 파일 구분자 : File.separator,System.getProperty("file.separator") 로 쓸수 있다
